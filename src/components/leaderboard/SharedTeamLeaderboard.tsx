...
interface TeamLeaderboardComponentProps {
  event: Tour | Tournament;
  teamLeaderboard: any[];
  sortedRounds: any[];
}

const SharedTeamLeaderboard: React.FC<TeamLeaderboardComponentProps> = ({
  event,
  teamLeaderboard,
  sortedRounds,
}) => {
  ...
  const isMatchPlay = event.rounds?.some(
              ...
              return (
                ...
                >
                  <TableCell sx={styles.tournamentLeaderboard.centeredDataCell}>
                    {position}
                    {index === 0 && event.status === "completed" && (
                      <Chip
                        label="Winner"
                        color="primary"
                        size="small"
...
export default SharedTeamLeaderboard;
